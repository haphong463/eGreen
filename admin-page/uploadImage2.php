...
function uploadImages($uploadedImages, $takeid)
{
    ...
    $uploadPath = "../image/accessory/";
    if (!is_dir($uploadPath)) {
        mkdir($uploadPath);
    }
    $uploadPathThumb = "../image/accessory/thumbnail/";
    ...
    foreach ($files as $file) {
        ...
        if ($file != false) {
            ...
            $image = 'image/accessory/'  . $file['name'] . '';
            $sql = 'insert into image2(acc_id, image) values (' . $takeid . ',"' . $image . '")';
            execute($sql);
            //tạo thumbnail
            $thumbnail = "../$image";
            $resize = new ResizeImage($thumbnail);
            $resize->resizeTo(561, 705, 'exact');
            //up file vật lý thumnail
            $resize->saveImage($uploadPathThumb . '/' . $file['name']);
            //luu đường dẫn vào db
            $thumbnail = 'image/accessory/thumbnail/' . $file['name'] . '';
            $sql = 'insert into thumbnail2(acc_id, thumbnail_path) values (' . $takeid . ',"' . $thumbnail . '")';
            execute($sql);
        }
    }
    return $errors;
