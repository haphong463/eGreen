...
$order_id = isset($_GET['cancel_return_order']) ? $_GET['cancel_return_order'] : header('Location: ../orders.php');

$sql = "UPDATE returns SET isAccept = -1 WHERE order_id = '$order_id'";
execute($sql);

$sql = "UPDATE orders SET status = 'Return canceled', updated_at = NOW() WHERE order_id = '$order_id'";
execute($sql);
